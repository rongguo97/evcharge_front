...
function ReservationPage() {
  ...
  const generateTimeSlots = (charger: any) => {
    ...
    let duration = 60; 
    if (typeLabel.includes("급속") || methodLabel.includes("100kW")) duration = 40;
  ...
  const handleConfirmReservation = async () => {
    try {
      ...
      // 예약 API 호출
      const response = await ReservationService.addReservation(reservationData as any);
      ...
      if (error.response && error.response.status === 409) alert(error.response.data.message || "이미 예약된 시간입니다.");
                ...
                <>
                  ...
                  {totalPages > 1 && (
                    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: "15px", gap: "8px", background: "#fafafa", borderTop: "1px solid #eee" }}>
                      <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} style={{ padding: "6px 12px", borderRadius: "8px", border: "1px solid #ddd", background: currentPage === 1 ? "#f0f0f0" : "#F5E6FF", color: currentPage === 1 ? "#aaa" : "black", fontWeight: "600", cursor: currentPage === 1 ? "default" : "pointer" }}>이전</button>
                      {pageNumbers.map((page) => (
                        <button key={page} onClick={() => setCurrentPage(page)} style={{ width: "32px", height: "32px", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "bold", background: currentPage === page ? "#B452B5" : "transparent", color: currentPage === page ? "#fff" : "#555" }}>{page}</button>
                      ))}
                      <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} style={{ padding: "6px 12px", borderRadius: "8px", border: "1px solid #ddd", background: currentPage === totalPages ? "#f0f0f0" : "#F5E6FF", color: currentPage === totalPages ? "#aaa" : "black", fontWeight: "600", cursor: currentPage === totalPages ? "default" : "pointer" }}>다음</button>
                    </div>
                  )}
                </>
              ...
              {chargers.length > 0 ? chargers.map((ch, index) => {
                ...
                return (
                  <div key={ch.chargerId || index}>
                    {/* 개별 충전기 버튼 */}
                    <button
  onClick={() => { setSelectedCharger(isSelected ? null : ch); setSelectedTime(null); }}
  style={{
    width: "100%", textAlign: "left", cursor: "pointer",
    padding: "12px", borderRadius: "12px", marginBottom: "8px",
    background: isSelected ? "#ffffff" : "#f8f9ff",
    border: isSelected ? "1px solid #f0f0f0" : "1px solid #eaddff",
    boxShadow: isSelected ? "0 4px 10px rgba(180, 82, 181, 0.15)" : "none",
  }}
>
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
    <strong style={{ color: "#4a3b8a", fontSize: "13px" }}>
      {ch.chargerName || `${ch.chargerId}호기`}
    </strong>
    <span style={{ 
      color: getStatusLabel(ch.status) === "충전가능" ? "#28a745" : "#7a5de8", 
      fontWeight: "800", 
      fontSize: "11px", 
  ...
  <div style={{ color: "#666", fontSize: "11px", marginTop: "5px" }}>
    {getTypeLabel(ch.chargerType)} | {getMethodLabel(ch.chargerMethod)}
                    ...
                    {isSelected && (
                      <div style={{ margin: "10px 0 20px 0", padding: "20px", background: "#fcfcfc", borderRadius: "12px", border: "1px solid #eaddff" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "15px" }}>
                          <h4 style={{ fontSize: "14px", fontWeight: "800", color: "#333", margin: 0 }}>🕒 예약 가능 시간 <span style={{ fontSize: "11px", color: "#B452B5", fontWeight: "normal" }}>({isFast ? "* 40분 충전" : "* 60분 충전"})</span></h4>
                          
                          {/* 날짜 선택 달력 래퍼 */}
                          <div style={{ position: "relative" }}>
                            <button onClick={(e) => { e.stopPropagation(); setIsCalendarOpen(!isCalendarOpen); }} style={{ padding: "6px 12px", borderRadius: "10px", border: "1px solid #eaddff", background: "#f8f9ff", color: "#B452B5", fontWeight: "700", cursor: "pointer", fontSize: "13px" }}> {moment(selectedDate).format('YYYY-MM-DD')}</button>
                            {isCalendarOpen && (
                              <div style={{ position: "absolute", top: "35px", right: 0, zIndex: 1000, boxShadow: "0 10px 25px rgba(0,0,0,0.15)", borderRadius: "12px", overflow: "hidden", backgroundColor: "#fff" }}>
                                <Calendar onChange={(date) => { handleDateChange(date); setIsCalendarOpen(false); }} value={selectedDate} minDate={new Date()} formatDay={(_, date) => moment(date).format('D')} />
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>
                          {generateTimeSlots(ch).map((time) => (
                            <button key={time} onClick={() => setSelectedTime(time)} style={{ padding: "10px 0", borderRadius: "8px", border: selectedTime === time ? "1px solid #eaddff" : "1px solid #eee", background: selectedTime === time ? "#F5E6FF" : "#fff", color: "#555", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>{time}</button>
                          ))}
                        </div>
                      </div>
                    )}
        ...
        <button 
          ...
          style={{ width: "100%", padding: "20px", borderRadius: "18px", border: "none", background: (!selectedStation || !selectedCharger || !selectedTime) ? "#ccc" : "linear-gradient(135deg, #B452B5 0%, #7a5de8 100%)", color: "#fff", fontSize: "18px", fontWeight: "900", cursor: "pointer" }}
        >
      ...
      {isModalOpen && selectedStation && selectedCharger && selectedTime && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0, 0, 0, 0.6)", zIndex: 999, display: "flex", justifyContent: "center", alignItems: "center", padding: "20px" }}>
          <div style={{ background: "#fff", padding: "40px", borderRadius: "24px", width: "100%", maxWidth: "500px", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" }}>
              ...
              <>
                ...
                <div style={{ background: "#f8f9ff", borderRadius: "12px", padding: "25px", border: "1px solid #eaddff", marginBottom: "30px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
                    ...
                    <strong style={{ color: "#333", fontSize: "16px" }}>
                      {selectedCharger?.chargerName || `${selectedCharger?.chargerId}호기`}
                    </strong>
                  </div>

                  {/* 예약 날짜 (moment 형식 변환 적용) */}
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
                    <span style={{ color: "#666", fontSize: "15px" }}>예약 날짜</span>
                    <strong style={{ color: "#333", fontSize: "16px" }}>{moment(selectedDate).format('YYYY-MM-DD')}</strong>
                  </div>

                  {/* 예약 시간 */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#666", fontSize: "15px" }}>예약 시간</span>
                    <strong style={{ color: "#B452B5", fontSize: "20px" }}>{selectedTime}</strong>
                  </div>
                </div>

                <p style={{ textAlign: "center", color: "#333", fontSize: "16px", fontWeight: "bold", margin: "0 0 30px 0" }}>
                  해당 정보로 예약을 진행하시겠습니까?
                </p>
                ...
                <div style={{ display: "flex", gap: "15px" }}>
                  <button 
                    ...
                    style={{ flex: 1, padding: "18px", borderRadius: "14px", border: "1px solid #ddd", background: "#fff", color: "#333", fontWeight: "bold", cursor: "pointer" }}
                  >
                    취소
                  </button>
                  <button 
                    onClick={() => setModalStep(2)} 
                    style={{ flex: 1, padding: "18px", borderRadius: "14px", border: "none", background: "linear-gradient(135deg, #B452B5 0%, #7a5de8 100%)", color: "#fff", fontSize: "16px", fontWeight: "bold", cursor: "pointer" }}
                  >
                    예약하기
                  </button>
                </div>
            ...
            {modalStep === 2 && (
              <>
                ...
                <div style={{ background: "#f8f9ff", borderRadius: "12px", padding: "25px", border: "1px solid #eaddff", marginBottom: "30px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" , color: "#333", }}><span>예약 금액 :</span><strong>{MOCK_RESERVATION_FEE.toLocaleString()} 원</strong></div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" , color: "#333", }}><span>현재 적립금 :</span><strong>{MOCK_RESERVATION_FEE.toLocaleString()} 원</strong></div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" , color: "#333", }}><span>차감 금액 :</span><strong style={{ color: "#DC143C" }}> -{remainPoints.toLocaleString()} 원</strong></div>
                  <div style={{ display: "flex", justifyContent: "space-between" , color: "#333", }}><span>결제 후 잔액 :</span><strong style={{ color: "#B452B5" }}>{remainPoints.toLocaleString()} P</strong></div>
                </div>
                <div style={{ display: "flex", gap: "15px" }}>
                  <button onClick={() => setModalStep(1)} style={{ flex: 1, padding: "18px", borderRadius: "14px", border: "1px solid #ddd", background: "#fff", color: "#333", fontWeight: "bold", cursor: "pointer", }}>이전</button>
                  <button onClick={handleConfirmReservation} style={{ flex: 1, padding: "18px", borderRadius: "14px", border: "none", background: "linear-gradient(135deg, #B452B5 0%, #7a5de8 100%)", color: "#fff", fontSize: "16px", fontWeight: "bold", cursor: "pointer"  }}>예약 확정</button>
                </div>
              </>
            )}
            ...
            {modalStep === 3 && (
              <div style={{ textAlign: "center" }}>
                ...
                <button onClick={() => window.location.href = '/'} style={{ width: "100%", padding: "18px", borderRadius: "14px", border: "none", background: "linear-gradient(135deg, #B452B5 0%, #7a5de8 100%)", color: "#fff", fontWeight: "bold" }}>홈으로 돌아가기</button>
              </div>
            )}
